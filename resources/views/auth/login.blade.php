<x-guest-layout>
    <div class="flex min-h-screen items-center justify-center bg-slate-200 px-6 py-10">
        <div class="w-full max-w-md rounded-xl bg-white px-8 py-8 shadow">
                ...
                <h1 class="mt-4 text-center text-3xl font-bold text-slate-900">Klinik Eva Mulia</h1>
                <x-auth-session-status class="mt-6" :status="session('status')" />

                <form method="POST" action="{{ route('login') }}" class="mt-5 space-y-4 rounded-lg border border-slate-200 p-5">
                    @csrf
                    <div>
                    ...
                    <p class="text-sm text-slate-600">
                        Belum punya akun?
                        <a href="{{ route('register') }}" class="font-semibold text-blue-700 hover:underline">Daftar di sini</a>
                    </p>
                </form>
        </div>
    </div>
